...
#include "include/7z_lib_main.h"
#include "lzma_c/7z.h"
#include "lzma_c/7zAlloc.h"
#include "lzma_c/7zCrc.h"
#include "lzma_c/7zFile.h"
#include "lzma_c/7zVersion.h"
...
void PrintError(FunType callback, char *sz)
{
  callback("\nERROR: %s\n", sz);
}
...
int lzma_lib_main(int numargs, char *args[], FunType callback)
{
  ...
  if (numargs < 3)
  {
    PrintError(callback, "incorrect command");
    return 1;
  }
  ...
  {
    PrintError(callback, "can not open input file");
    return 1;
  }
    ...
    if (res == SZ_OK)
    {
      ...
      */
      ...
      for (i = 0; i < db.db.NumFiles; i++)
      {
        ...
        SzArEx_GetFileNameUtf16(&db, i, temp+5);
		memcpy(temp, args[3], strlen(args[3]));  //cswuyg
        if (listCommand)
        {
        ...
        if (!testCommand)
        {
          ...
          {
            PrintError(callback, "can not open output file");
            res = SZ_ERROR_FAIL;
            break;
          }
          ...
          {
            PrintError(callback, "can not write output file");
            res = SZ_ERROR_FAIL;
            break;
          }
          if (File_Close(&outFile))
          {
            PrintError(callback, "can not close output file");
            res = SZ_ERROR_FAIL;
            break;
          }
          #ifdef USE_WINDOWS_FILE
  ...
  if (res == SZ_ERROR_UNSUPPORTED)
    PrintError(callback, "decoder doesn't support this archive");
  else if (res == SZ_ERROR_MEM)
    PrintError(callback, "can not allocate memory");
  else if (res == SZ_ERROR_CRC)
    PrintError(callback, "CRC error");
  else
    callback("\nERROR #%d\n", res);
  return 1;
}
